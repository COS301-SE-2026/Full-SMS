...
def create_plugin(request: PluginCreate, current_user: CurrentUser):
    return create_plugin_controller(request, current_user["id"])


@router.put("/{plugin_id}", summary="Update a plugin")
def update_plugin(plugin_id: str, request: PluginUpdate, current_user: CurrentUser):
...
def toggle_plugin(plugin_id: str, request: PluginToggle, current_user: CurrentUser):
