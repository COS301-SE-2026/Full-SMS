...
async def init_upload(payload: dict, current_user: Annotated[dict, Depends(get_current_user)]):
    return await init_hdf5_upload(payload, current_user)

@router.post("/uploads/{upload_id}/complete")
async def complete_upload(upload_id: str, payload: dict, current_user: Annotated[dict, Depends(get_current_user)]):
...
async def upload_result(upload_id: str, current_user: Annotated[dict, Depends(get_current_user)]):
    return await get_hdf5_upload_result(upload_id, current_user)

@router.get("/user-uploads")
async def get_user_uploads( current_user: Annotated[dict, Depends(get_current_user)]):
