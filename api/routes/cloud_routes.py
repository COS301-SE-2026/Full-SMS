...
def upload_from_onedrive(payload: OneDriveUploadPayload, current_user:  Annotated[dict, Depends(get_current_user)]):
    return onedrive_upload_controller(payload=payload, user_id= current_user["user"]["id"])