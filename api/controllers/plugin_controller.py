...
def create_plugin_controller(request: PluginCreate, user_id: str) -> dict:
    try:
        ...
        return {"success": True, "message": "Plugin created successfully", "plugin": plugin}
    except ValueError as valerror:
        raise HTTPException(
...
def update_plugin_controller(plugin_id: str, request: PluginUpdate, user_id: str) -> dict:
    try:
        config_dict = None
        ...
        return {"success": True, "message": "Plugin updated successfully", "plugin": plugin}
    except ValueError as valerror:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail=str(valerror))
    except Exception as e:
...
def toggle_plugin_controller(plugin_id: str, request: PluginToggle, user_id: str) -> dict:
    try:
        plugin = toggle_plugin(plugin_id, user_id, request.enabled)
        status_text = "enabled" if request.enabled else "disabled"
        return {"success": True, "message": f"Plugin {status_text} successfully", "plugin": plugin}
    except ValueError as valerror:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail=str(valerror))
    except Exception as e:
